...
def parse_comment_date(s: str):
    """
    Formato esperado: '30 Dec, 2024 @ 12:13pm'
    """
    s = s.strip()
    try:
...
def build_review_edges():
    ...
    def flush(game, entries):
        for i in range(len(entries)):
            u_curr, d_curr, _ = entries[i]
            
            start = max(0, i - WINDOW)

            for j in range(start, i):  # todos os anteriores
                u_prev, d_prev, t_prev = entries[j]
...
def build_comment_edges(comment_edges_raw: list) -> list:
    """
    Estratégia de cadeia (idêntica à de reviews):
      - Ordena os comentários de cada perfil por data.
      - O dono do perfil conecta com o PRIMEIRO comentarista.
      - Cada comentarista conecta com o PRÓXIMO (i → i+1).
    """
    print("  Construindo arestas de comentário (cadeia por perfil) ...")

    # Agrupa por dono do perfil
    ...
    for owner, entries in por_perfil.items():
        ...
        for i in range(len(chain)):
            dst, dst_date, _ = chain[i]
            
            start = max(0, i - WINDOW)
            
            for j in range(start, i):  # todos os anteriores
                src, src_date, src_text = chain[j]
