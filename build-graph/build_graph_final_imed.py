...
def build_review_edges(scores_map: dict):
    ...
    def flush(game, entries):
        for i in range(len(entries) - 1):
            u1, d1, t1, rd1 = entries[i]
            u2, d2, t2, rd2 = entries[i + 1]
            if u1 == u2:
                continue
            key = (u1, u2, game)
            if key not in seen:
                ...
                hash_src = get_text_hash(u1, t1, rd1)
                hash_dst = get_text_hash(u2, t2, rd2)
...
def build_comment_edges(comment_edges_raw: list, scores_map: dict) -> list:
    ...
    for owner, entries in por_perfil.items():
        ...
        # Estrutura da cadeia: (usuário, data_iso, texto_original, data_original)
        chain = [(owner, None, "", "")] + [(e["author"], e["date"], e["text"], e["raw_date"]) for e in entries_sorted]

        for i in range(len(chain) - 1):
            src, src_date, src_text, src_raw_date = chain[i]           
            dst, dst_date, dst_text, dst_raw_date = chain[i + 1]
